...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 19 21:22:21 2026
...
NEW (your request):
- HMM trend loop (t-2 -> t) per team:
  We run the HMM on:
    t   : all matches
    t-1 : drop most recent match
    t-2 : drop 2 most recent matches
  Convert each run’s next-match probs into Expected Points (EP = 3*Pwin + 1*Pdraw).
  Use EP series to estimate form trend and a % change from t-2 to t.
- Interactive efficiency adjustment choice:
    1) No: keep historical play learnings (0% adjustment)
    2) Yes: apply HMM-derived efficiency change (clamped to [-50, +50])
    3) Manual: user enters adjustment % per team (as before)
- External mean adjustment knob for simulation generation still works:
  mean_used = mean * (1 + adj/100), std unchanged, applied in generate_team_test().
"""
...
def ask_efficiency_choice() -> int:
    ...
    """
    print("Do you want to adjust team efficiencies based on HMM trend?")
    print("  1) no, keep the historical play learnings")
    print("  2) yes, apply the calculated performance changes")
    print("  3) yes, I want to adjust myself")
...
def run_models(df_engineered: pd.DataFrame):
    ...
    # guard against duplicate columns before anything else
...
def run_simulation_perspective(
    ...
    # outcome prediction uses OUTCOME_BASE_FEATURES (+ interactions inside pipeline)
...
def fit_best_hmm(X, k_min=2, k_max=3, random_state: int = 42):
    """
    NOTE: uses a stable random_state so results are less jittery run-to-run.
    """
    best_bic = np.inf
...
def _run_markov_for_team_drop_recent(team_name: str, drop_recent: int) -> Dict[str, float]:
    """
    Same HMM logic, but drop the most-recent N matches BEFORE building the chronological sequence.

    Your file is most recent -> oldest, so dropping top rows removes newest games.
    """
    team1_file = MAIN_FOLDER / team_name / "mixed-seasons" / f"{team_name}_Games_Input.xlsx"
    if not team1_file.exists():
        raise FileNotFoundError(f"Missing file for {team_name}: {team1_file}")
    ...
    if len(df_raw) < 6:
        # too little data to make a meaningful trend; return neutral-ish
        return {
            ...
            "P_draw": 1/3,
            "P_loss": 1/3,
            "EP": expected_points_from_probs({"P_win": 1/3, "P_draw": 1/3, "P_loss": 1/3}),
        }
...
def compute_hmm_efficiency_change(team_name: str) -> dict:
    """
    Compare 4 games total (t-3, t-2, t-1, t) by dropping most recent rows:
      t   : drop_recent=0
      t-1 : drop_recent=1
      t-2 : drop_recent=2
      t-3 : drop_recent=3
    ...
    We compute stepwise % changes:
      c1 = EP(t-3) -> EP(t-2)
      c2 = EP(t-2) -> EP(t-1)
      c3 = EP(t-1) -> EP(t)

    Final efficiency change = mean(c1, c2, c3)
    """
    ...
    # simple monotonic trend label (optional, for printing)
    downward = (ep_t < ep_t1 < ep_t2 < ep_t3)
    upward = (ep_t > ep_t1 > ep_t2 > ep_t3)
    trend = "down" if downward else ("up" if upward else "mixed")

    # Suggested adjustment to simulation mean (same scale as manual input)
    ...
    return {
        ...
        "step_changes_pct": [float(c1), float(c2), float(c3)],
...
def print_efficiency_trend(team_name: str, trend_info: dict):
    ep_t3, ep_t2, ep_t1, ep_t = trend_info["ep_series"]
    delta = trend_info["avg_change_pct"]
    trend = trend_info["trend"]

    if delta < 0:
        direction = "less efficient"
        mag = abs(delta)
    elif delta > 0:
    ...
    tag = ""
    if trend == "down":
        tag = " (clear downward trend)"
    elif trend == "up":
        tag = " (clear upward trend)"
    else:
        tag = " (mixed trend)"

    print(f"Form trend (HMM EP) for {team_name}:")
    print(f"  EP(t-2)={ep_t2:.3f} -> EP(t-1)={ep_t1:.3f} -> EP(t)={ep_t:.3f}")
    if direction == "about the same":
        print(f"  {team_name} is analyzed to be {direction} over the last 3 games{tag}.")
    else:
        print(f"  {team_name} is analyzed to be playing {direction} by {mag:.1f}% over the last 3 games{tag}.")
    print(f"  Suggested simulation mean adjustment: {trend_info['suggested_adj_pct']:+.1f}%\n")
...
def _weighted_goals_from_topN(score_df: pd.DataFrame) -> tuple[float, float, int]:
    """
    score_df must have columns: Score, Count
    Returns (exp_goals_team1, exp_goals_team2, total_count_used)
    using count-weighted averages over the provided rows.
    """
...
def combine_perspectives(home_sim: dict, away_sim: dict) -> dict:
    """
    home_sim: Team1 = Home, Team2 = Away  -> score strings are Home-Away
    away_sim: Team1 = Away, Team2 = Home  -> score strings are Away-Home

    We:
      1) average W/D/L as before
      2) compute expected goals by count-weighted averaging of top scorelines
         from both perspectives, after reversing away_sim into Home-Away orientation.
    """
    p1 = home_sim["probs"]
    p2 = away_sim["probs"]
    ...
    # --- NEW SCORE COMBINATION LOGIC (weighted using top5 scorelines) ---

    # From home_sim: Team1=Home, Team2=Away
    exp_h1, exp_a1, n1 = _weighted_goals_from_topN(home_sim.get("top5_scores"))

    # From away_sim: Team1=Away, Team2=Home (Away-Home)
    exp_away_as_team1, exp_home_as_team2, n2 = _weighted_goals_from_topN(away_sim.get("top5_scores"))

    # Convert away_sim orientation (Away-Home) into Home-Away:
    exp_h2 = exp_home_as_team2
    exp_a2 = exp_away_as_team1

    # Blend the two perspectives.
    # Recommended: weight by the amount of evidence (sum of counts) from each side.
    if (n1 + n2) > 0:
...
def main():
    ...
    # 1) Run latest HMM and print findings as-is
    markov_home = run_markov_for_team(home_team)
    markov_away = run_markov_for_team(away_team)

    # 2) Run the change algorithm (t-2 -> t) and print trend for both teams
    ...
    print("1.3 Recent-form trend (HMM-based efficiency, last 3 games)\n")
    print_efficiency_trend(home_team, trend_home)
    print_efficiency_trend(away_team, trend_away)

    # 3) Ask whether to adjust team efficiencies
    ...
    if choice == 1:
        ...
        print("\nManual efficiency adjustments selected.\n")
    ...
    # 4) Simulations with chosen adjustments
    ...
    # home_sim: Team1 = home_team, Team2 = away_team
    t1_avgs = home_sim["avg_player_ratings"]["team1_players"]
    t2_avgs = home_sim["avg_player_ratings"]["team2_players"]
    
    print("Average simulated player ratings (per player, across all simulations):")
    print(f"{home_team} (Team1Player1..11):")
    for i in range(1, 12):
        k = f"Team1Player{i}"
        v = t1_avgs.get(k, float("nan"))
        print(f"  {k}: {v:.2f}")
    
    print(f"\n{away_team} (Team2Player1..11):")
    for i in range(1, 12):
        k = f"Team2Player{i}"
        v = t2_avgs.get(k, float("nan"))
        print(f"  {k}: {v:.2f}")
